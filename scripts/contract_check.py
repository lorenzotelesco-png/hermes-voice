...
Run it on the server before and after every `hermes update`:
    ...
    python3 scripts/contract_check.py --chat     # also runs one tiny real completion
...
CHECKS = [
    ...
    ("api", "/v1/capabilities", ["features.session_chat_streaming", "features.run_approval_response",
                                 "features.run_stop"], 1),
    ("dash", "/api/sessions?limit=1", ["sessions", "total"], 1),
...
def main():
    ...
    if "--chat" in sys.argv:
        body = json.dumps({"model": os.environ.get("HERMES_MODEL", "hermes-agent"), "max_tokens": 20,
                           "messages": [{"role": "user", "content": "Rispondi solo con la parola: pronto"}]}).encode()
        status, raw = get(API_URL, {**headers["api"], "Content-Type": "application/json"}, body)
        ok = status == 200 and b"choices" in raw
        failed += not ok
        print(f"{'ok  ' if ok else 'FAIL'}  fase 0  api  POST /v1/chat/completions" + ("" if ok else f"  -> HTTP {status}"))
